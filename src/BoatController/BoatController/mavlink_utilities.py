...
def set_position(connection, destination):
    connection.mav.send(mavutil.mavlink.MAVLink_set_position_target_global_int_message(10, connection.target_system,
                        connection.target_component, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, int(0b110111111000), int(destination[0] * 10 ** 7), int(destination[1] * 10 ** 7), 20, 0, 0, 0, 0, 0, 0, 1.57, 0.5))
    print("For debugging: command sent to mav")
...
def setX_velocity_and_yaw(connection,boot_time ,Vx,Yaw):
   connection.mav.set_position_target_global_int_send(
        ...
        type_mask=(0b100111100111), lat_int=0, lon_int=0, alt=15, # (x, y WGS84 frame pos - not used), z [m]
        vx=Vx, vy=0, vz=0, # velocities in NED frame [m/s] (not used)
...
def control_rc_channels(connection, chan1, chan2):
    ...
    connection.mav.rc_channels_override_send(
            ...
            chan1,  # chan1_raw (e.g., roll)
            1500,  # chan2_raw (e.g., pitch)
            1500,   # chan3_raw (e.g., throttle)
            chan2,   # chan4_raw (e.g., right motor)
            0,      # chan5_raw
            0,      # chan6_raw
            0,      # chan7_raw
            0       # chan8_raw
        )
    #print(f"Override sent at {counter} seconds.")
    #sleep(1)
    #counter += 1
    #return
